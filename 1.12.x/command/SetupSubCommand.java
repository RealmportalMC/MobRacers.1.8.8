...
public class SetupSubCommand extends SubCommand
{
	...
	public boolean onSubCommand(CommandSender sender, String label, String[] args)
	{
		if(args.length == 0)
		{
			...
			sender.sendMessage(CourseMessage.COMMAND_CONFIG_MENU_VEHICLES.toString("<status>", "§f(" + game.getSetup().getVehicleLocations().size() + " / " + VehicleType.values().length + ")"));
			...
			else
			{
				...
				if(game.getSetup().getExit() == null && !game.getConfig().isAutoJoin() && !game.getConfig().keepPlayerStates())
					CourseMessage.COMMAND_CONFIG_MENU_NOTREADY_NOEXIT.say(sender);
				
				if(game.getSetup().getVehicleLocations().size() == 0)
					CourseMessage.COMMAND_CONFIG_MENU_NOTREADY_NOVEHICLES.say(sender);
				
				if(game.getSetup().isBadRegion())
		...
		if(args[0].equalsIgnoreCase("vehiclelist"))
		{
			...
			for(VehicleType type : VehicleType.values())
				sender.sendMessage("  " + (game.getSetup().getVehicleLocations().containsKey(type) ? "§a" : "§c") + type.name().toLowerCase().replaceAll("_", "-"));
			return true;
		}
		...
		if(args[0].equalsIgnoreCase("setvehicle"))
		{
			if(sender instanceof Player)
			{
				...
				game.getSetup().getVehicleLocations().put(type, new SerializableLocation(((Player) sender).getLocation()));
		...
		{
			game.getSetup().getVehicleLocations().clear();
			CourseMessage.COMMAND_CONFIG_VEHICLESET.say(sender);
			return true;
		}
		...
		{
			for(VehicleType type : MobRacersPlugin.getGame().getSetup().getVehicleLocations().keySet())
			{
				Vehicle vehicle = type.createNewVehicle();
				vehicle.spawn(game.getSetup().getVehicleLocations().get(type).getLocation(), null);
				Entity entity = vehicle.getEntity();
				if(entity == null)
					continue;
				EntityUtil.setNoAI(entity, true);
				EntityUtil.setYawPitch(game.getPlayers(), entity, game.getSetup().getVehicleLocations().get(type).getLocation().getYaw(), game.getSetup().getVehicleLocations().get(type).getLocation().getPitch());
		...
		if(args[0].equalsIgnoreCase("tovehicle") || args[0].equalsIgnoreCase("tpvehicle"))
		{
			...
			{	VehicleType type = VehicleType.valueOf(args[1].toUpperCase().replaceAll("-", "_"));
				...
				SerializableLocation serialLoc = game.getSetup().getVehicleLocations().get(type);
		...
		if(args[0].equalsIgnoreCase("removevehicle"))
		{
			...
			game.getSetup().getVehicleLocations().remove(type);
		...
		if(args[0].equalsIgnoreCase("setleaveitem"))
		{
			...
			game.getSetup().setLeaveItem(player.getItemInHand());
		...
		if(args[0].equalsIgnoreCase("setarenaitem"))
		{
			...
			game.getSetup().setArenaItem(player.getItemInHand());
			CourseMessage.COMMAND_CONFIG_ITEMSET.say(sender);
			return true;
		}
		...
		if(args[0].equalsIgnoreCase("setvehicleitem"))
		{
			...
			game.getSetup().setVehicleItem(player.getItemInHand());
			CourseMessage.COMMAND_CONFIG_ITEMSET.say(sender);
			return true;
		}
		...
		if(args[0].equalsIgnoreCase("setspectatoritem"))
		{
			...
			game.getSetup().setSpectatorItem(player.getItemInHand());
			CourseMessage.COMMAND_CONFIG_ITEMSET.say(sender);
			return true;
		}
